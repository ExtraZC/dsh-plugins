...
（`/home/extraz/.dsh/scripts/restart-with-confirm.sh`）。
...
```bash
# 1. 用带确认流程的新单元文件替换旧的（先备份）
sudo cp /etc/systemd/system/dsh-web-restart.service /etc/systemd/system/dsh-web-restart.service.bak
sudo install -m 644 \
  "/home/extraz/DSH/安装/dsh-restart-confirm/systemd/dsh-web-restart.service" \
  /etc/systemd/system/dsh-web-restart.service
...
sudo touch /home/extraz/.dsh/profiles/web/package.json
# 此时页面应出现确认栏；也可直接查接口：
